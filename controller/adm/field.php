...
class Field_Controller extends Adm
{
	function main()
	{
		...
		$this->field = $field->fetch('field_module != ' . $this->module->getId('user'), 'field_module, field_order')->fetch();
		$this->count = array();

		foreach ($this->field as $row)
		{
			if (!isset($this->count[$row['module_id']]))
			{
				$this->count[$row['module_id']] = 1;
			}
			else
			{
				$this->count[$row['module_id']] += 1;
	...
	public function submit($id = 0)
	{
		...
		if ($this->input->isMethod(Input::POST))
		{
			...
			if ($this->filter->isValid($_POST) && $formValidation)
			{
				...
				{
					$data += array(
						'field_module'			=> (int)$this->post->module,
						'field_component'		=> (int)$this->post->component
					);
				...
				$field->filter->setFieldFilters($id, $this->post->filter);

				if ($this->post->item)
				{
					$field->item->setFieldItems($id, $this->post->item);
				}
				...
				if ($data)
				{
					$field->option->setFieldOptions($id, $data);
				}

				$this->redirect('adm/Field');
			}
		}
		...
		return View::getView('adm/fieldSubmit', $result);
	}
}
?>