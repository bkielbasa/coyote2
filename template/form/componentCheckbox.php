...
<!--
	...
	$(document).ready(function()
	{
		...
		{
			$('<li><input class="item" type="text" name="item[name][]" /> = <input class="item" type="text" name="item[value][]" /> <button type="button" onclick="deleteItem(this);">Usuń opcję</button></li>').appendTo('#optionList');
...
<li>
	...
	<fieldset id="optionList">
		...
		<ol>
			...
			<li><?= Form::input('item[name][]', $name, array('class' => 'item')); ?> = <?= Form::input('item[value][]', $value, array('class' => 'item')); ?> <button type="button" onclick="deleteItem(this);">Usuń opcję</button></li>
			<?php endforeach; ?>			
		</ol>
	</fieldset>
</li>
...
<li>
	<label>&nbsp;</label>
	<?= Form::button('insertField', 'Dodaj nowe pole'); ?>
</li>