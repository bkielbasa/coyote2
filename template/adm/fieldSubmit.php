...
<!--
	$(document).ready(function()
	{
		...
		{
			window.location.href = '<?= url("adm/Field/Submit"); ?>?componentId=' + $(this).val();
		}
		);

		<?php if (@$field_id) : ?>
		$('select[name=component]').attr('disabled', 'disabled');
		$('select[name=module]').attr('disabled', 'disabled');
		<?php endif; ?>
	});
//-->
</script>
	...
	<fieldset>
		...
		<ol>
			<li>
				...
				<?= Form::select('module', Form::option($modules, $input->post('module', @$field_module))); ?>
			</li>
			<li>
