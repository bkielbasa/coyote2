...
<h1>Niestandardowe pola</h1>

<p>Tutaj możesz utworzyć dodatkowe, niestandardowe pola w określonych modułach.</p>
...
<?= Form::open(url('adm/Field/Submit')); ?>
	<fieldset>
		<legend>Utwórz nowe pola</legend>
...
<?= Form::open('', array('method' => 'post', 'onsubmit' => 'return confirm(\'Czy na pewno chcesz usunąć zaznaczone pola?\');')); ?>
	<table>
		...
		<tbody>
			...
			<tr>
				<td><?= Html::a(url('adm/Field/Submit/' . $row['field_id'] . '?componentId=' . $row['field_component']), $row['field_id']); ?></td>
				<td><?= Html::a(url('adm/Field/Submit/' . $row['field_id'] . '?componentId=' . $row['field_component']), $row['field_text']); ?></td>
				<td><?= $row['component_text']; ?></td>		
				<td><?= $row['module_text']; ?></td>
				<td><?= img_sort($row['field_id'], $row['field_order'], $count[$row['module_id']]); ?></td>
				<td class="checkbox"><?= Form::checkbox('delete[]', $row['field_id'], false); ?></td>
			</tr>
			<?php endforeach; ?>
			<?php else : ?>
			<tr>
				<td colspan="6" style="text-align: center;">Brak dodatkowych pól w bazie danych.</td>
			</tr>
			<?php endif; ?>
		</tbody>
		<tfoot>
